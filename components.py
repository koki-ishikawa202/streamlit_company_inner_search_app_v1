...
def display_initial_ai_message():
    ...
    with st.chat_message("assistant"):
        ...
        st.markdown("<span style='color:green; background-color:#e8f5e9; padding:10px; border-radius:5px; display:block; width:100%;'>こんにちは。私は社内文書の情報をもとに回答する生成AIチャットボットです。サイドバーで利用目的を選択し、画面下部のチャット欄からメッセージを送信してください。</span>", unsafe_allow_html=True)
        st.markdown("<span style='color:#a0522d; background-color:#fff3cd; padding:10px; border-radius:5px; display:block; width:100%;'>⚠︎ 具体的に入力した方が期待通りの回答を得やすいです。</span>", unsafe_allow_html=True)
